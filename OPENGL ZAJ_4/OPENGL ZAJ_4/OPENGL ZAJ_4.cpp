...
void GenerateTextures()
{
	...
	GLboolean error = load_targa("old_man_white_skin_beard_20141210_2027176126.tga", width, height, format, type, pixels);

	// błąd odczytu pliku
	if (error == GL_FALSE)
	{
		printf("Niepoprawny odczyt pliku old_man_white_skin_beard_20141210_2027176126.tga");
		exit(0);
	}
	...
	error = load_targa("old_man_white_skin_beard_gray.tga", width, height, format, type, pixels);

	// błąd odczytu pliku
	if (error == GL_FALSE)
	{
		printf("Niepoprawny odczyt pliku old_man_white_skin_beard_gray.tga");
		exit(0);
	}
